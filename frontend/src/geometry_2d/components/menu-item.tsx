  ...
  return (
    <div className="flex gap-1">
      ...
      <div className="relative">
        ...
        {open && (
          <div className="flex flex-col gap-1 absolute border-2 border-charade-950 rounded bg-bej-100 px-1 py-2  top-0 -translate-x-1/3 -translate-y-[calc(100%_+_10px)] ">
            {itemMenu.map((item, index) => {
              return (
                <button
                  key={item.action}
                  className="flex items-center w-full hover:bg-carnation-400 px-1 py-[2px] cursor-pointer rounded"
                >
                  <Icon
                    name={item.icon}
                    width={20}
